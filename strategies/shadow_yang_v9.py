...
class ShadowYangStrategy(Strategy):
    """影线后收阳线策略 V9（趋势再入场增强版）。

    V9 在 V2 基础上增加趋势再入场机制：
    策略在强趋势中卖出后，如果价格回踩 MA10 后继续创新高且多头排列保持，
    则再次入场捕捉趋势延续。

    数据驱动：
    - 24 笔止盈清仓平均 +19.57%，是策略核心利润来源
    - 但很多大趋势单卖飞后就再也没回来（如 600539 主升浪中只做了 1 段）
    - 趋势再入场可以捕捉同一波趋势的多个波段
    """

    # 状态机:
    #   WAITING --买入--> HOLDING --止损/止盈--> WAITING
    #   WAITING --趋势再入场--> HOLDING
    ...
    def init(self) -> None:
        ...
        # V9：趋势再入场状态跟踪
        self._last_exit_bullish: bool = False  # 上次卖出时是否为多头排列
        self._last_exit_price: float = 0.0
        self._trend_reentry_cooldown: int = 0  # 再入场冷却
    ...
    def next(self) -> None:
        ...
        if self._state == _WAITING:
            ...
            # V9：趋势再入场检查（优先于普通信号）
            if self._check_trend_reentry(cur_close, cur_open):
                return
    ...
    def _check_trend_reentry(self, cur_close: float, cur_open: float) -> bool:
        """趋势再入场：多头排列 + 回踩MA10后新高。

        条件：
        1. 上次卖出时为多头排列（说明从趋势中出来的）
        2. 当前仍然是多头排列
        3. 价格回踩过 MA10（中间至少有过1天收盘价<MA10）
        4. 今日阳线且创近 10 日新高（趋势延续确认）
        5. 不在冷却期

        返回 True 表示已执行买入（不再检查其他信号）。
        """
        if not self._last_exit_bullish:
            return False
        if self._trend_reentry_cooldown > 0:
            self._trend_reentry_cooldown -= 1
            return False
        ...
        if not self._is_ma_bullish(i):
            self._last_exit_bullish = False  # 趋势破坏了，重置
            return False
        ...
        # 中间必须有过回踩（至少1天收盘价低于MA10）
        ma10_touched = False
        for j in range(1, min(20, i + 1)):
            if self.data.close[-j] < self.ma10[i - j]:
                ma10_touched = True
                break
        if not ma10_touched:
            return False
        ...
        # 今日创近 10 日新高（突破确认）
        recent_high = max(self.data.high[-j] for j in range(1, 11))
        cur_high = self.data.high[0]
        ...
        # 执行再入场
        atr = self._calc_atr(i)
        self.buy(size=0, price=cur_close, reason="趋势再入场")
        self._state = _HOLDING
        self._entry_price = cur_close
        self._highest_since_entry = cur_close
        self._highest_close_since_entry = cur_close
        self._entry_atr = atr
        self._entry_adx = self._adx[i]
        self._entry_bullish = True
        self._holding_days = 0
        self._prev_day_yin = False
        self._sell_signal_pending = False
        self._last_exit_bullish = False  # 重置
        return True
    ...
    def _exit_position(self, price: float, is_loss: bool, reason: str = "") -> None:
        ...
        # V9：记录卖出时是否为多头排列（用于趋势再入场判断）
        i = self._bar_index
        self._last_exit_bullish = self._is_ma_bullish(i)
        self._last_exit_price = price
        self._trend_reentry_cooldown = 3  # 冷却 3 天
