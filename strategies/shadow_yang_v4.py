...
ATR_PERIOD = 14
ATR_STOP_MULTIPLIER = 2.5  # V4: 2.2 → 2.5，放宽止损给趋势单更多空间
MAX_LOSS_PCT = 9.0  # V4: 8.0 → 9.0，配合放宽止损
...
# V4: 精简盈利锁定（3级阶梯）
# Tier3: 10% 触发锁 5% / Tier4: 15% 触发锁 8% / Tier5: 20% 触发锁 12%
# 移除原 Tier1(3%保本) 和 Tier2(6%锁2%)，让更多单子发展成大趋势
PROFIT_LOCKIN_TRIGGER1 = 0.10  # V4: 0.03 → 0.10，原 Tier1 跳空到 Tier3
PROFIT_LOCKIN_TRIGGER2 = 0.15  # V4: 0.06 → 0.15
PROFIT_LOCKIN_FLOOR2 = 0.08    # V4: 0.02 → 0.08
PROFIT_LOCKIN_TRIGGER3 = 0.20  # V4: 0.10 → 0.20
PROFIT_LOCKIN_FLOOR3 = 0.12    # V4: 0.03 → 0.12
PROFIT_LOCKIN_TRIGGER4 = 0.25  # 保留
PROFIT_LOCKIN_FLOOR4 = 0.15    # V4: 0.07 → 0.15
PROFIT_LOCKIN_TRIGGER5 = 0.30  # V4: 0.20 → 0.30
PROFIT_LOCKIN_FLOOR5 = 0.20    # V4: 0.10 → 0.20

# ── 强趋势盈利锁定豁免 ──────────────────────────────────────────────────────
# V4: 强趋势从 Tier2(15%) 开始锁定（原 Tier4 对应）
# PROFIT_LOCKIN_STRONG_TREND_MIN_TIER = 3 表示从第3档（15%）开始
PROFIT_LOCKIN_STRONG_TREND_MIN_TIER = 2  # V4: 3 → 2，强趋势从 15% 档开始
...
class ShadowYangStrategy(Strategy):
    ...
    def _check_buy(self, cur_close: float, cur_open: float, prev_low: float) -> None:
        ...
        # V4: 弱反弹过滤（仅针对「MA5上行」这一最弱触发条件）
        # 阳线实体 < 前日阴线 × 0.6 时，视为弱反弹假信号，不买入
        # 数据驱动：-3~0% 的小亏交易（55笔，占亏损57%）很多是弱反弹后小幅回落
        is_weak_rebound = False
        if (
            is_prev_yin and is_yang
            and is_ma5_up  # 仅 MA5上行 的最弱分支
            and not is_bullish_engulfing  # 看涨吞没不受限
            and not is_strong_reversal_yang  # 强反转不受限
        ):
            prev_body = prev_open - prev_close
            cur_body = cur_close - cur_open
            if prev_body > 0 and cur_body < prev_body * 0.6:
        ...
        if (
            ...
            and not is_weak_rebound  # V4: 弱反弹过滤
        ):
    ...
    def _calc_profit_floor(
        ...
        # V4: 强趋势豁免前两阶（Tier1/Tier2 已合并进 Tier3 起点）
        # 非强趋势下，10% 是第一档锁定
        if max_profit >= PROFIT_LOCKIN_TRIGGER2:
            # V4: PROFIT_LOCKIN_TRIGGER2=15% 与 PROFIT_LOCKIN_TRIGGER3 重合
            # 此分支实际上不会触发，保留代码完整性
            return self._entry_price * (1 + PROFIT_LOCKIN_FLOOR2)
        if max_profit >= PROFIT_LOCKIN_TRIGGER1:
            # V4: PROFIT_LOCKIN_TRIGGER1=10%，即新的 Tier3 起点
            # 强趋势下豁免此档
            if is_entry_strong_trend and PROFIT_LOCKIN_STRONG_TREND_MIN_TIER >= 3:
                return 0.0
            return self._entry_price * (1 + PROFIT_LOCKIN_FLOOR2)  # 第一档锁 8%
