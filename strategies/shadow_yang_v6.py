...
PROFIT_LOCKIN_TRIGGER1 = 0.05  # V6: 0.03 → 0.05，5% 才保本（给更多空间）
PROFIT_LOCKIN_TRIGGER2 = 0.10  # V6: 0.06 → 0.10，10% 锁 4%
PROFIT_LOCKIN_FLOOR2 = 0.04    # V6: 0.02 → 0.04
PROFIT_LOCKIN_TRIGGER3 = 0.15  # V6: 0.10 → 0.15，15% 锁 7%
PROFIT_LOCKIN_FLOOR3 = 0.07    # V6: 0.03 → 0.07
PROFIT_LOCKIN_TRIGGER4 = 0.25  # V6: 0.15 → 0.25，25% 锁 12%
PROFIT_LOCKIN_FLOOR4 = 0.12    # V6: 0.07 → 0.12
PROFIT_LOCKIN_TRIGGER5 = 0.35  # V6: 0.20 → 0.35，35% 锁 18%
PROFIT_LOCKIN_FLOOR5 = 0.18    # V6: 0.10 → 0.18
...
CCI_OVERBOUGHT = 150  # V6: 100 → 150，CCI 100-150 不是极端超买，是趋势延续
...
class ShadowYangStrategy(Strategy):
    ...
    def _check_buy(self, cur_close: float, cur_open: float, prev_low: float) -> None:
        ...
        # V6: 买入日上影线过滤（阳线冲高回落 = 上方抛压重，假信号多）
        # 数据驱动：55 笔 -3~0% 小亏交易中，很多是长上影阳线上冲失败后回落
        # 上影线 / 实体 > 0.8 时不买入（仅对影线后收阳主路径）
        if is_yang:
            cur_body = cur_close - cur_open
            if cur_body > 0:
                upper_shadow = cur_high - cur_close
                if upper_shadow > cur_body * 0.8:
                    return
