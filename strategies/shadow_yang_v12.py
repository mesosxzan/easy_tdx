...
class ShadowYangStrategy(Strategy):
    """影线后收阳线策略 V12（V10 + 缩量阴线过滤）。

    在 V10（MA20>MA60中期过滤）基础上，增加缩量阴线过滤：
    前日阴线必须是缩量的（量 < 5日均量），缩量阴线 = 抛压减弱，反转更可靠。
    放量阴线 = 恐慌抛售/主力出货，后续阳线反弹更可能是诱多。
    """
    ...
    def _check_buy(self, cur_close: float, cur_open: float, prev_low: float) -> None:
        ...
        # V12：前日阴线缩量过滤
        # 影线后收阳策略要求前日为阴线，放量阴线 = 恐慌抛售/主力出货，
        # 后续阳线更可能是诱多反弹；缩量阴线 = 抛压减弱，反转更可靠
        # 例外：强反转阳线（实体 >= 前日阴线 × 2.0）不受限（极端买盘力量）
        if i >= 5 and is_prev_yin and not is_strong_reversal_yang:
            prev_vol = self.data.vol[-1]
            vol_ma5_prev = sum(self.data.vol[-j] for j in range(2, 7)) / 5.0  # 不含当日
            if vol_ma5_prev > 0 and prev_vol > vol_ma5_prev * 1.0:
                return  # 前日阴线放量，不买入
