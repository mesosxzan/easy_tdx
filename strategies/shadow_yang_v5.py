...
# 逐步收紧锁定利润。
PROFIT_TIER2 = 0.03  # V5: 0.05 -> 0.03，浮盈 3% 即启动追踪止盈
...
# ── V5 精简盈利锁定（仅3级，从 12% 开始）────────────────────────────────
# 数据驱动：31 笔盈利锁定平均仅 +0.64%（45% 胜率），贡献极低但截断了
# 很多本可以奔跑的趋势单。移除低阶锁定后，更多单子会进入追踪止盈。
# Tier1: 12% 浮盈锁 6% / Tier2: 20% 锁 10% / Tier3: 30% 锁 15%
PROFIT_LOCKIN_TRIGGER1 = 0.12  # 12% 开始第一档锁定（V2 是 3%）
PROFIT_LOCKIN_TRIGGER2 = 0.20  # 20% 第二档
PROFIT_LOCKIN_FLOOR2 = 0.10   # 锁 10%
PROFIT_LOCKIN_TRIGGER3 = 0.30  # 30% 第三档
PROFIT_LOCKIN_FLOOR3 = 0.15   # 锁 15%
PROFIT_LOCKIN_TRIGGER4 = 0.40  # 40% 第四档
PROFIT_LOCKIN_FLOOR4 = 0.22   # 锁 22%
PROFIT_LOCKIN_TRIGGER5 = 0.50  # 50%+
PROFIT_LOCKIN_FLOOR5 = 0.30   # 锁 30%

# ── V5 强趋势盈利锁定豁免 ────────────────────────────────────────────────
# 强趋势从 Tier2(20%) 开始锁定
PROFIT_LOCKIN_STRONG_TREND_MIN_TIER = 2  # V5: 3 → 2（强趋势从第2档才开始）
...
class ShadowYangStrategy(Strategy):
    ...
    def _check_buy(self, cur_close: float, cur_open: float, prev_low: float) -> None:
        ...
        # V5: 纯 MA5上行 的弱反弹必须放量确认（减少 -3~0% 小亏交易）
        # 数据驱动：55 笔 -3~0% 小亏交易中，很多是无量弱反弹后回落
        is_ma5_only = (
            is_ma5_up and not is_bullish_engulfing and not is_strong_reversal_yang
        )
        if is_ma5_only:
            if i >= 5:
                vol_now = self.data.vol[0]
                vol_ma5_v5 = sum(self.data.vol[-j] for j in range(1, 6)) / 5.0
                if vol_ma5_v5 > 0 and vol_now < vol_ma5_v5 * 1.2:
                    return
    ...
    def _calc_profit_floor(
        ...
        """V5 精简版盈利锁定（仅 3 级有效，从 12% 开始）。

        设计逻辑：让盈利单充分奔跑，仅在大幅盈利后才锁定。
        数据驱动：V2 中 31 笔盈利锁定平均仅 +0.64%，大量趋势单被过早截断。
        ...
        if max_profit >= PROFIT_LOCKIN_TRIGGER1:
            # 强趋势豁免：强趋势下从第 2 档(20%)才开始锁定
            if is_entry_strong_trend and PROFIT_LOCKIN_STRONG_TREND_MIN_TIER >= 2:
                return 0.0
            return self._entry_price * (1 + PROFIT_LOCKIN_FLOOR2 * 0.6)  # 第一档锁 6%
        return 0.0
