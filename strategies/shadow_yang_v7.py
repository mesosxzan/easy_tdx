...
PROFIT_LOCKIN_TRIGGER1 = 0.03  # 收盘浮盈达 3%：锁定保本（止损移至买入价）
PROFIT_LOCKIN_TRIGGER2 = 0.06  # 收盘浮盈达 6%：锁定 2% 利润
PROFIT_LOCKIN_FLOOR2 = 0.02
PROFIT_LOCKIN_TRIGGER3 = 0.10  # 收盘浮盈达 10%：锁定 3% 利润
PROFIT_LOCKIN_FLOOR3 = 0.03
PROFIT_LOCKIN_TRIGGER4 = 0.15  # 收盘浮盈达 15%：锁定 7% 利润
...
_WAITING = "WAITING"
_HOLDING = "HOLDING"
_PENDING_PULLBACK = "PENDING_PULLBACK"  # V7：等待回踩入场


class ShadowYangStrategy(Strategy):
    """影线后收阳线策略 V7（强趋势回调入场优化版）。

    V7 核心优化：强趋势/多头排列中信号后等回踩 MA5 再入场。

    数据驱动：
    - 强趋势中阳线追高买入成本偏高，回踩 MA5 可降低成本 2-5%
    - 非强趋势中不宜等回调（可能直接下跌，错过反转机会）
    - 仅在 ADX>=30 或多头排列时启用回调入场

    状态机::
        WAITING --信号触发(强趋势)--> PENDING_PULLBACK --回踩--> HOLDING
          \\--信号触发(非强趋势)--> HOLDING
    """
    ...
    def init(self) -> None:
        ...
        # V7：回调入场状态
        self._pullback_wait_bars: int = 0
        self._pullback_signal_reason: str = ""
        self._pullback_signal_atr: float = 0.0
        self._pullback_max_wait = 5  # 最多等 5 根 K 线
    ...
    def next(self) -> None:
        ...
        # ── V7：等待回踩入场 ────────────────────────────────────────────────
        if self._state == _PENDING_PULLBACK:
    ...
    def _check_buy(self, cur_close: float, cur_open: float, prev_low: float) -> None:
        ...
        if (
            ...
            # V7：强趋势/多头排列中等待回踩 MA5 再入场（降低成本）
            if is_strong_trend or is_bullish:
                self._enter_pullback_mode(buy_reason + "(待回踩)", atr, i)
            else:
                self._do_buy(cur_close, atr, buy_reason, i)
            return
        elif is_yang:
                ...
                if not is_strong_trend:
                    ...
                    if (
                        ...
                        if boll_pctb_bull > BOLL_PCTB_BULLISH_MAX:
                            return
                atr = self._calc_atr(i)
                # V7：多头排列形成信号也等回踩 MA5 再入场
                self._enter_pullback_mode("多头排列形成(待回踩)", atr, i)

    # ── V7：回调入场辅助方法 ───────────────────────────────────────────────

    def _enter_pullback_mode(self, reason: str, atr: float, i: int) -> None:
        """进入等待回踩状态。"""
        self._state = _PENDING_PULLBACK
        self._pullback_wait_bars = 0
        self._pullback_signal_reason = reason
        self._pullback_signal_atr = atr
        # 记录信号日的 ADX 和多头排列状态（用于入场时设置）
        self._pullback_entry_adx = self._adx[i]
        self._pullback_entry_bullish = self._is_ma_bullish(i)
    ...
    def _check_pullback_entry(self, cur_close: float) -> None:
        ...
        # 超时放弃（强趋势中连续上涨不回调，直接追入）
        if self._pullback_wait_bars > self._pullback_max_wait:
            # 超时后直接买入（避免错过主升浪）
            atr = self._calc_atr(i)
            self._do_buy(cur_close, atr, self._pullback_signal_reason + "(超时)", i)
            return

        # 回踩 MA5 触发（收盘价 <= MA5 × 1.01）
        ma5_trigger = ma5_now * 1.01
        if cur_close <= ma5_trigger:
            atr = self._calc_atr(i)
            self._do_buy(cur_close, atr, self._pullback_signal_reason, i)
