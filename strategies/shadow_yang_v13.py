...
class ShadowYangStrategy(Strategy):
    """影线后收阳线策略 V10（中期趋势过滤增强版）。

    V10 核心改进：MA20 必须在 MA60 之上才交易（中期多头排列过滤）。

    数据驱动（V2 30股回测）：
    - 最大亏损交易（300750 -21.6%、603127 -18.9%、601899 -15.9%）均发生在
      MA20 在 MA60 下方的下跌趋势中
    - MA60 方向过滤（近20日跌幅 < 0.5%）不够严格，下跌中继中 MA60 可能
      走平但 MA20 已跌破 MA60，此时抄底风险极高
    - MA20 > MA60 = 中期趋势向上，是反转策略的安全垫
    """
