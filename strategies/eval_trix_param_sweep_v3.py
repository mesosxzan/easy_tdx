"""TRIX 优化策略参数扫描 V3 -- 放松入场过滤 + 更高 ATR。

V2 发现：
  - ATR=6.0/MA15/MA20/MA60 仍为最优 (+10.32%, 15.39% dd)
  - ATR≥8.0 零交易（超 max_value=6.0 被跳过）
  - 中位收益 -2.73%（多数股票亏损，平均靠少数大赢家拉高）

V3 新增：
  - skip_bounds=True 允许 ATR 超过 6.0
  - volatility_lookback 作为维度（大值→入场过滤放松）
  - 测试 ATR=[5.0, 6.0, 8.0, 10.0, 15.0]
  - 固定 ma_fast=15, ma_mid=20, ma_trend=60

用法::

    python strategies/eval_trix_param_sweep_v3.py
"""
...
# V3 网格
ATR_MULTS = [5.0, 6.0, 8.0, 10.0, 15.0]
VOL_LOOKBACKS = [20, 40, 60, 99]
# 固定最优: ma_fast=15, ma_mid=20, ma_trend=60
...
def run_combo(
    ...
    for symbol, market_str, name in EVAL_STOCKS:
        ...
        if df is None:
            continue
        try:
            # skip_bounds=True 允许超出 Param.min/max 的值
            strategy = strategy_cls(skip_bounds=True, **params)
    ...
    if cr.valid_count > 0:
        ...
        returns.sort()
        cr.median_return = returns[len(returns) // 2] if returns else 0.0
...
def main(top: int = 20) -> None:
    total_combos = len(ATR_MULTS) * len(VOL_LOOKBACKS)
    print("=" * 140)
    print("  TRIX 优化策略参数扫描 V3 -- 放松入场过滤 + 更高 ATR")
    print(f"  标的数: {len(EVAL_STOCKS)} 只 | K线: {KLINE_COUNT} 根")
    print("  固定: ma_fast=15, ma_mid=20, ma_trend=60")
    print(f"  扫描维度: atr_mult={ATR_MULTS}")
    print(f"            volatility_lookback={VOL_LOOKBACKS} (大值→入场过滤放松)")
    print(f"  总组合数: {total_combos}")
    print("  使用 skip_bounds=True 允许超范围值")
    print("=" * 140)
    ...
    for atr_mult, vol_lb in product(ATR_MULTS, VOL_LOOKBACKS):
        combo_idx += 1
        params = {
        ...
        filter_note = "放松" if vol_lb >= 60 else ("标准" if vol_lb == 20 else "中等")
        print(
            f"  [{combo_idx}/{total_combos}] ATR={atr_mult:>4} vol_lb={vol_lb:>2}  "
            ...
            f"正收益={cr.positive_count}/{cr.valid_count} "
            f"评分={cr.composite_score * 100:+.2f} [{filter_note}]"
    ...
    header = (
        f"  {'排名':>4} {'ATR':>5} {'vol_lb':>6} "
        f"{'均收益':>9} {'中位收益':>9} {'回撤':>8} {'夏普':>7} {'胜率':>7} "
        f"{'持仓天':>6} {'正收益':>7} {'跑赢':>6} {'交易':>5} {'评分':>8} {'过滤':>6}"
    )
    print(header)
    print(f"  {'-' * 120}")
    ...
    for rank, cr in enumerate(sorted_combos[:top], 1):
        ...
        if cr.volatility_lookback >= 60:
            filter_note = "放松"
        elif cr.volatility_lookback == 20:
            filter_note = "标准"
        else:
            filter_note = "中等"
        print(
            f"  {rank:>4} {cr.chandelier_atr_mult:>5.1f} {cr.volatility_lookback:>6} "
            ...
            f"{cr.total_trades:>5} "
            f"{cr.composite_score * 100:>+7.2f} {filter_note:>6}{marker}"
        )

    # ── 维度分析 ──
    print("\n  --- ATR 倍数影响 (固定 vol_lb=20) ---")
    for atr in ATR_MULTS:
        matching = [
            c for c in combos
            if c.chandelier_atr_mult == atr and c.volatility_lookback == 20
        ]
        if matching and matching[0].valid_count > 0:
            c = matching[0]
            print(f"    ATR={atr:>4.1f}: 收益={c.avg_return * 100:+.2f}%  "
                  f"中位={c.median_return * 100:+.2f}%  "
                  f"回撤={c.avg_drawdown * 100:.2f}%  交易={c.total_trades}  "
                  f"持仓={c.avg_holding_days:.1f}d  正收益={c.positive_count}/{c.valid_count}  "
                  f"评分={c.composite_score * 100:+.2f}")

    print("\n  --- 波动率回溯影响 (固定 ATR=8.0) ---")
    for vlb in VOL_LOOKBACKS:
        matching = [
            c for c in combos
            if c.chandelier_atr_mult == 8.0 and c.volatility_lookback == vlb
        ]
        if matching and matching[0].valid_count > 0:
            c = matching[0]
            filter_note = "放松" if vlb >= 60 else ("标准" if vlb == 20 else "中等")
            print(f"    vol_lb={vlb:>2}: 收益={c.avg_return * 100:+.2f}%  "
                  f"中位={c.median_return * 100:+.2f}%  "
                  f"回撤={c.avg_drawdown * 100:.2f}%  交易={c.total_trades}  "
                  f"持仓={c.avg_holding_days:.1f}d  正收益={c.positive_count}/{c.valid_count}  "
                  f"评分={c.composite_score * 100:+.2f}  [{filter_note}]")

    # 最优 vs 基线
    best = sorted_combos[0] if sorted_combos and sorted_combos[0].valid_count > 0 else None
    if best:
        print("\n  --- 最优组合 vs 原版 TRIX ---")
        print(f"  最优参数: ATR={best.chandelier_atr_mult}  vol_lb={best.volatility_lookback}  "
              f"MA15/MA20/MA60")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="TRIX 优化策略参数扫描 V3")
