...
from easy_tdx.MyTT import BOLL, CCI, DMI, KDJ, MA, MFI
...
@register_strategy(
    name="shadow_yang_v2",
    label="影线后收阳线V2",
    description="基于影线后收阳线反转逻辑的数据驱动优化版。"
    "包含 MACD/CCI/MFI/BOLL/KDJ 多因子超买过滤、ADX 趋势识别、"
    "ATR 动态止损、分阶段追踪止盈、盈利锁定阶梯等优化。全仓进出。",
)
class ShadowYangV2Strategy(ParametrizedStrategy):
    """影线后收阳线策略 V2（优化版）。
    ...
    # ── 引擎级需求 ──
    required_position_mode = "full"
    default_warmup_bars = 60  # MA60 预热

    # 无可配置参数（全部硬编码常量，保留回测验证的最优值）
    params: ClassVar[list[Param]] = []

    def init(self) -> None:
        self.ma5 = self.I(MA, self.data.close, 5)
        self.ma10 = self.I(MA, self.data.close, MA_STOP_PERIOD)
        self.ma20 = self.I(MA, self.data.close, MA_LONG_PERIOD)
        self.ma60 = self.I(MA, self.data.close, MA_VERY_LONG_PERIOD)
        # ADX 震荡市过滤
        self._pdi, self._mdi, self._adx, self._adxr = self.I(
            DMI, self.data.close, self.data.high, self.data.low
        )
        # 入场质量过滤指标（CCI/MFI/BOLL 超买检测）
        self._cci = self.I(CCI, self.data.close, self.data.high, self.data.low, CCI_PERIOD)
        self._mfi = self.I(
            MFI,
            self.data.close,
            self.data.high,
        ...
        self._boll_upper, self._boll_mid, self._boll_lower = self.I(
            BOLL, self.data.close, BOLL_PERIOD, BOLL_DEVIATION
        )
        # KDJ 极端超买过滤（J > 100 时全部亏损）
        self._kdj_k, self._kdj_d, self._kdj_j = self.I(
            KDJ, self.data.close, self.data.high, self.data.low, KDJ_PERIOD, 3, 3
        )
