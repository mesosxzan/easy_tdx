...
def DSMA(X, N):  # 偏差自适应移动平均线   type: (np.ndarray, int) -> np.ndarray
    ...
    for i in range(len(X)):
        Filt[i] = c1 * (Zeros[i] + Zeros[i - 1]) / 2 + c2 * Filt[i - 1] + c3 * \
                  Filt[i - 2]
...
def SAR(HIGH, LOW, N=10, S=2, M=20):
    ...
    for i in range(N, length):
        if b_first:  # 第一步
            ...
            if (is_long and HIGH[i] > ep) or (
                    (not is_long) and LOW[i] < ep):  # 顺势：多创新高 或者 空创新低
                af = min(af + f_step, f_max)
            #
            sar_x[i] = sar_x[i - 1] + af * (ep - sar_x[i - 1])

        if (is_long and LOW[i] < sar_x[i]) or (
                (not is_long) and HIGH[i] > sar_x[i]):  # 反空 或者 反多
            is_long = not is_long
            b_first = True
    return sar_x


def TDX_SAR(High, Low, iAFStep=2,
        iAFLimit=20):  # type: (np.ndarray, np.ndarray, int, int) -> np.ndarray
